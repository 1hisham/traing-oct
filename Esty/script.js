...
function totalPopularGiftsNow() {
  fetch(
    ...
    .then((popularGiftsNow) => {
      ...
      let totalPriceArray = calculationDiscount();

      function forArrayPrices(searchValue) {
        let index = [];
        let total = [];
        total = totalPriceArray;
        console.log(searchValue);
        total.filter((Element, idx) => {
      ...
      buttonInput.addEventListener("click", () => {
        ...
        filterArray = forArrayPrices(searchValue);
      });
      
      console.log(filterArray,"filer");
      ...
      function popularGiftCardSingle(
        ...
        // let filterClass = document.querySelectorAll('.full-card')
        if (1 > 5) {
          fullCard.classList.add("full-card-filter");
        }
      }
